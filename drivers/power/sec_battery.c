...
struct battery_info {
	...
	u32 abnormal_current_consumption;	/* 0 : Not active 1: Active */
...
static void sec_bat_status_update(struct power_supply *bat_ps)
{
	...
	if (battery->info.abnormal_current_consumption)
		sec_get_bat_level(bat_ps);
	else
	...
	pr_info("BAT : soc(%d), vcell(%dmV), curr(%dmA), temp(%d.%d), chg(%d), full(%d), rechg(%d), sudden(%d), cable(%d)\n",
		battery->info.level,
		...
		battery->info.abnormal_current_consumption,
		battery->current_cable_status);
...
static void sec_bat_work(struct work_struct *work)
{
	...
	if (battery->info.abnormal_current_consumption) {
		...
		diff_soc = battery->info.level - current_soc;
		if (diff_soc < -STABLE_LOW_BATTERY_DIFF) {
			pr_info("%s: back to normal current consumption\n",
				__func__);
			battery->info.abnormal_current_consumption = 0;
		} else if (diff_soc > STABLE_LOW_BATTERY_DIFF) {
			pr_info("%s: soc is going down\n", __func__);
...
int _low_battery_alarm_(struct battery_data *battery)
{
	...
	if ((battery->info.level - current_soc) > STABLE_LOW_BATTERY_DIFF) {
		pr_info("%s: abnormal current consumption\n", __func__);
		battery->info.abnormal_current_consumption = 1;
